---
type: species
scientific_name: "Meleagris gallopavo"
common_name: "Turkey"
taxonomic_class: "Aves"
primary_habitat: "Forest"
curation_score: 10
priority_tier: 3
iucn_status: "Least Concern (LC)"
poaching_risk_score: 1
protected_breeding_zone: "Yukon Protected Buffer"
patrol_frequency_days: 13
security_clearance: "RESTRICTED"
last_assessment_date: "2026-05-13"
---

# Turkey (*Meleagris gallopavo*)

**Taxonomic Class:** Aves  
**Primary Habitat:** Forest  

**Curation Score:** 10/100
**Priority Tier:** 3

## 🛡️ Tier 3: Conservation Intelligence
- **IUCN Status:** Least Concern (LC)
- **Poaching Risk Score:** 1/10
- **Protected Breeding Zone:** Yukon Protected Buffer
- **Ranger Patrol Frequency:** Every 13 days
- **Security Clearance Requirement:** `RESTRICTED`

### Conservation Measures & Protocols
Active tracking program deployed in Yukon Protected Buffer. Strict anti-poaching measures enforced. Periodic thermal surveillance.

## 🕸️ Tier 2: Relational Network & Ecological Interactions
### Species Interactions
*No interactions documented.*

### Connected Corridors
- [[Yukon-Rocky_Mountain_Wilderness_Corridor]]

## 📡 Tier 1: Raw Observation & Telemetry Data
*No telemetry observations logged.*
