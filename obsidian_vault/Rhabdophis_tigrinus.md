---
type: species
scientific_name: "Rhabdophis tigrinus"
common_name: "Tiger Keelback"
taxonomic_class: "Reptilia"
primary_habitat: "Wetlands"
curation_score: 15
priority_tier: 3
iucn_status: "Not evaluated (NE)"
poaching_risk_score: 2
protected_breeding_zone: "Russia range"
patrol_frequency_days: 12
security_clearance: "RESTRICTED"
last_assessment_date: "2026-01-05"
---

# Tiger Keelback (*Rhabdophis tigrinus*)

**Taxonomic Class:** Reptilia  
**Primary Habitat:** Wetlands  

**Curation Score:** 15/100
**Priority Tier:** 3

## 🛡️ Conservation Intelligence
- **IUCN Status:** Not evaluated (NE)
- **Poaching Risk Score:** 2/10
- **Protected Breeding Zone:** Russia range
- **Ranger Patrol Frequency:** Every 12 days
- **Security Clearance Requirement:** `RESTRICTED`

### Conservation Measures & Protocols
Population trend: Unknown. Primary diet: Carnivore. Typical lifestyle: Semiaquatic, Predator,.

## 🕸️ Relational Network & Ecological Interactions
### Species Interactions
- **Predation** relationship with [[Batagur_baska]] (Northern River Terrapin)
  - *Energy Pathway:* Diet-inferred trophic link
  - *Details:* Tiger Keelback (Carnivore) preys on species matching its dietary profile, including Northern River Terrapin.

### Connected Corridors
- [[Eurasian_Forest_Migration_Network]]

## 📡 Raw Observation & Telemetry Data
| Date/Time | Location (Lat, Long) | Sensor ID | Temp (°C) | Battery | Image Reference | Observations / Notes |
| --- | --- | --- | --- | --- | --- | --- |
| 2026-08-22 07:24 UTC | 38.5159, 121.5157 | `FIELD-CAM-757` | 37.5°C | 27.40% | N/A | Simulated field observation within Asia range (no real telemetry source for this dataset). |
